...
## 範圍（2026-09-01 補完輪後：引擎原語 P1–P16 全數落地）

- **有**：P1 registry+audit｜P2 組（含臨時組合）｜P3 採集｜P4 上游（gh api，無事不報）｜P5 增量 digest（便宜模型）｜P6 打包簡版｜P7 兩段式碰撞｜P8 落｜P9 生（升格＋出生登記＋outcome 回連）｜P10 脊椎三件套｜P11 查詢＋品味量測（命中率＋存活率）｜P12 排程（timer 住殼內、當日補課跨日不補）｜P13 通知（interrupt 白名單分類）｜P15 provider（mock＋claude，judge＋自由文字）｜P16 會話（帶料開終端＋MCP 掛載）｜**MCP server**（stdio 零依賴，agent 的介面＝人的介面）｜早晨簡報產生器（樣貌 A 純事實版）｜S4 監控台網頁雛形（`ui`，有 schedule 時內建 S6 timer）。
...
repoengine/
  ...
  registry.py  P1 registry+audit ＋ P2 組（含臨時組合）＋ 存活率視圖；寫入同一把 lock
  collect.py   P3 git 狀態（dirty 天數、末 commit 天數）
