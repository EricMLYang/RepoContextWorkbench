...
- **2026-09-25 Agent 友善輪「離 Agent 友善還很遠」**→ 定性為**主角錯位**：工具以工作台為主、agent 為客。
  改成 agent 為主要使用者（`docs/20260925_Agent友善輪.md`）。搜尋測試順手抓到舊 bug：`git ls-files` 沒加 `-z`，
  中文檔名 md 被跳脫成帶引號路徑而全部略過（打包／文件地圖／碰撞都受影響）→ 補回歸測試再修。
- **2026-09-25 跨 repo 參考輪「常請 agent 去參考另一個 repo」**→ 定性為**缺一層「用名字引用」**：
  AGENTS.md 寫死路徑、整批複製、agent 不知道對方哪裡有料（`docs/20260925_跨repo參考輪.md`）。
  lint 在真資料第一次跑出 29 筆、其中 13 筆誤報（`<repo>/子路徑` 其實存在、`.../` 省略號）→
  先補 failing test 釘住「只報有把握的」再修，收斂到 16 筆、逐條人工核對皆為真。
  狀態卡在真資料抓到 dev server 啟動 9 天、之後改過 96 個檔——當天「要不要重開」的原題。
- **2026-09-25 「分群找不到如何新增或刪除 repo」**→ 定性為**缺口不是 bug**：組只有建立一個動作，
  加減組員／改名／刪組四個入口（CLI／工作台／MCP／registry）都沒有。先補 13 條 failing test（`tests/test_groups.py`）再做；
  刪組保留 `groups/<組>/` 的料、改名跟著搬、撞到既有資料夾就拒絕不合併；工作台編輯改名失敗不留半套；
  config 排程指到該組只回報不代改。截圖自看抓到一個體感問題：改名後的牌組卡跑到清單最後 → 卡片按 registry 順序就位。
- **2026-09-25 架構檢查「以可擴充性看要不要重構」**→ 定性為**寫入路徑有實錘 bug＋入口各寫一份**（`docs/20260925_架構檢查.md`）。① registry 只鎖寫入那一刻：2 進程各寫 40 筆只剩 41 → `transaction()`＋原子寫入，補不變式 `test_inv_registry_no_lost_updates`（舊的只驗沒撕裂、沒驗沒掉資料）。② 管理寫入收進 `ops.py` 操作表，CLI／工作台／MCP 共用驗證與留痕。③ cli.py／webui.py／registry.py 拆成套件，JS 格式化（esbuild 壓縮後逐位元相同）。重構中測試順手抓到兩個語意問題：CLI 改設定的留痕會進收件匣（→ source `[cli]`）、改設定被摘要當成「上次進度」（→ `〔registry〕` 前綴）。
- **2026-09-25 反芻輪「想一個讓效用加倍的功能」**→ 定性為**只有拉、沒有推**：知識要 agent 先想到去搜才用得上
  （`docs/20260925_反芻輪.md`）。真資料校準抓到三個門檻問題：idf 中位數在卡片庫貼著最大值（0 筆）、中文 bigram 跨詞邊界的假詞
  （「接是／得上」）湊過門檻、`.claude/`／`.agents/` 兩份相同 SKILL.md 混進推薦 → 修完各補一條測試釘住，定版 2 筆皆對題。
- **2026-09-26 回填輪「有什麼任務可以跑一整晚」**→ 定性為**冷啟動缺料**：新 spine 空的，反芻 0 筆（`docs/20260926_回填輪.md`）。
  真資料抓到三件：目標線索混進操作指令（→ 收緊 prompt）；使用者原本就有正式 spine、前一輪 setup 沒先查就另建一個
  （→ 依使用者裁定搬移後刪舊）；登記子專案後已抽的會話歸屬錯組（→ 先補 failing test 再修 remap）。另 `test_inv_no_private_names`
  抓到我在引擎註解裡寫了真 repo 名。
