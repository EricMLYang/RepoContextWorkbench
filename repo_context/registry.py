...
def add_group_members(spine_dir, name, ids):
    ...
    ids = _dedupe(ids)
    _check_members(data, ids)
    g["members"] += [i for i in ids if i not in g["members"]]
    save(spine_dir, data)
    return g
...
def remove_group_members(spine_dir, name, ids):
    ...
    g = _find_group(data, name)
    ids = _dedupe(ids)
    missing = [i for i in ids if i not in g["members"]]
    if missing:
        raise ValueError(f"不在組「{name}」裡: {', '.join(missing)}")
    g["members"] = [m for m in g["members"] if m not in ids]
    save(spine_dir, data)
    return g
...
def rename_group(spine_dir, old, new):
    ...
    new = _check_group_name(new)
    data = load(spine_dir)
    g = _find_group(data, old)
    if new == old:
        return {"group": g, "moved_dir": False, "schedule_refs": []}
    if any(x["name"] == new for x in data["groups"]):
        raise ValueError(f"組已存在: {new}")
    src, dst = Path(spine_dir) / "groups" / old, Path(spine_dir) / "groups" / new
    moved = src.is_dir()
    if moved and dst.exists():
        raise ValueError(f"資料夾已存在: groups/{new}/（先自行合併或移走，不代為覆蓋）")
    refs = _schedule_refs(spine_dir, old)
    g["name"] = new
    save(spine_dir, data)
    if moved:
        src.rename(dst)
    return {"group": g, "moved_dir": moved, "schedule_refs": refs}
