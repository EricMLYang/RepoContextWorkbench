...
開場注入若有「自上次以來跟你手上工作相關的新料」：那是排程在會話之間，拿本組的目標／未結／
最近判斷去比對**別組新進的知識與判斷**得到的，每筆附共同詞（為什麼推）。跟這次任務有關就
`read_from` 那個 ref（讀了會算「用上了」，推薦就會越調越準）；無關就略過，不必解釋。
要看完整清單：`ctx fresh --json`。
